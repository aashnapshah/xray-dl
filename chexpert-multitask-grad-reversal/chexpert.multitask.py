...
class ResNet(pl.LightningModule):
    def __init__(self, num_classes_disease,
                 num_classes_race, class_weights_race):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx): 
        loss_disease, loss_race = self.process_batch(batch)
        self.log_dict({"train_loss_disease": loss_disease, "train_loss_race": loss_race})
        wandb.log({"train_loss_disease": loss_disease, "train_loss_race": loss_race})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_dict({"val_loss_disease": loss_disease, "val_loss_race": loss_race})

    def test_step(self, batch, batch_idx):
        loss_disease, loss_sex, loss_race = self.process_batch(batch)
        self.log_dict({"test_loss_disease": loss_disease, "test_loss_race": loss_race})
...
def test(model, data_loader, device):
    ...
    with torch.no_grad():
        for index, batch in enumerate(tqdm(data_loader, desc='Test-loop')):
            ...
            out_disease, out_race = model(img)
...
def main(hparams):
    ...
    print('VALIDATION')
    preds_val_disease, targets_val_disease, logits_val_disease, preds_val_race, targets_val_race, logits_val_race = test(model, data.val_dataloader(), device)
    ...
    print('TESTING')
    preds_test_disease, targets_test_disease, logits_test_disease, preds_test_race, targets_test_race, logits_test_race = test(model, data.test_dataloader(), device)
